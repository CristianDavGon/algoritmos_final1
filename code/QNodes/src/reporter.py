...
def guardar_markdown(
    ...
    Args:
        resultados: Lista de dicts usada para el CSV.
                    ...
                    Claves KQNodes: también k, Criterio
        ruta_md: Ruta de salida del archivo .md.
        algoritmo: Nombre del algoritmo (ej. "QNodes", "KQNodes", "GeoMIP").
    ...
    es_kqnodes = bool(resultados) and "k" in resultados[0]
    ...
    for r in resultados:
        _append_caso(lines, r, es_kqnodes)
...
def fmt_particion_amigable(particion: str | None) -> str:
    ...
    - KQNodes k-partición:  múltiples biparticiones unidas con ' | '
...
def _norm(x: str) -> str:
    """Devuelve ∅ si el contenido está vacío, de lo contrario lo retorna tal cual."""
    stripped = x.strip()
...
def _parse_fmt_curvo(s: str) -> str:
    ...
    # Bipartición estándar (2 columnas): cada columna es una parte
    # KQNodes k-partición (2k columnas): columnas pares = parte izquierda relevante,
    # columnas impares = parte derecha trivial (∅)
...
def _append_caso(lines: list[str], r: dict, es_kqnodes: bool) -> None:
    prueba = r["Prueba"]
    alcance = r["Alcance"]
